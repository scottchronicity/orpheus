#!/usr/bin/env bash
# Installer for the Orpheus Audio Events Agent systemd unit.
#
# Deploys the agent into /opt/orpheus/agents/orpheus-agent-audio-events with
# an isolated Python 3.9 virtual environment and installs the service file.

set -euo pipefail

SERVICE_NAME="orpheus-agent-audio-events"
AGENT_ID="orpheus-agent-audio-events"
