#!/usr/bin/env bash
# Installer for the Orpheus Crow Detection Agent systemd unit.
#
# Deploys the agent into /opt/orpheus/agents/orpheus-agent-crow-detection with
# an isolated Python 3.9 virtual environment and installs the service file.

set -euo pipefail

SERVICE_NAME="orpheus-agent-crow-detection"
AGENT_ID="orpheus-agent-crow-detection"
...
setup_venv() {
  ...
  "${INSTALL_ROOT}/venv/bin/pip" install "pip==24.0"
