#!/usr/bin/env bash
# Installer for the Orpheus Audio Motion Detector systemd unit.
#
# Deploys the agent into /opt/orpheus/agents/orpheus-agent-audio-motionwith
# an isolated Python 3.9 virtual environment and installs the service file.

set -euo pipefail

SERVICE_NAME="orpheus-agent-audio-motion"
AGENT_ID="orpheus-agent-audio-motion"
