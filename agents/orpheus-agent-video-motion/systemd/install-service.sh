#!/usr/bin/env bash
# Installer for the Orpheus Video Motion Detection Agent systemd unit.
#
# Deploys the agent into /opt/orpheus/agents/orpheus-agent-video-motion with
# an isolated Python 3.9 virtual environment and installs the service file.

set -euo pipefail

SERVICE_NAME="orpheus-agent-video-motion"
AGENT_ID="orpheus-agent-video-motion"
