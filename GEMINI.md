# Gemini Code Assist instructions for Orpheus

**Read [`AGENTS.md`](./AGENTS.md) — it's the canonical entry point for
all AI coding agents in this repo (Claude, Copilot, Gemini, Cursor, etc.).**

`AGENTS.md` is short by design. It contains:

1. The non-negotiable rules (every one tied to a past incident
...
A critical hard rule for this project: it's locked to **Python 3.9.5**
for Jetson hardware compatibility. Do not suggest Python 3.10+ syntax
(`match/case`, `X | None` union syntax, etc.). See AGENTS.md for the
full list.
