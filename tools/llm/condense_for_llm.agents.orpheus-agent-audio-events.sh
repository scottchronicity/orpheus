#!/bin/bash
# condenses the orpheus-agent-audio-events project files into a single output file for LLM ingestion


SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
REPO_ROOT=$(cd "$SCRIPT_DIR/../.." && pwd)
cd "$REPO_ROOT/agents/orpheus-agent-audio-events"

OUTPUT_FILE="orpheus-agent-audio-events-condensed-for-llm.out"
