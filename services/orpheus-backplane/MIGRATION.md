...
> **Historical document** — this describes the November 2025 config-directory
> migration of the pre-rename `orpheus-mqtt` service, kept for the record. The
> `services/orpheus-mqtt` paths, `sudo make` invocations, and the `setup` branch
> below no longer exist: the service is now `services/orpheus-backplane` (NATS
> default, mosquitto fallback), `make install` must NOT be run with sudo (targets
> self-sudo), and current install steps live in [QUICKSTART.md](QUICKSTART.md) /
> [README.md](README.md). Do not follow the commands below on a current checkout.
