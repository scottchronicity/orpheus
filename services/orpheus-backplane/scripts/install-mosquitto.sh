...
SYSTEMD_SERVICE_SRC="${MQTT_BROKER_DIR}/systemd/orpheus-backplane-mosquitto.service"
SYSTEMD_SERVICE_DEST="/etc/systemd/system/orpheus-backplane.service"
...
# Remove any legacy orpheus-mqtt.service so it doesn't linger on :1883 alongside
# the new orpheus-backplane unit.
...
log_info "Enabling orpheus-backplane service..."
systemctl enable orpheus-backplane.service
log_success "Service enabled (will start on boot)"

# Step 8: Start the service
log_info "Starting orpheus-backplane service..."
systemctl start orpheus-backplane.service
...
if systemctl is-active --quiet orpheus-backplane.service; then
    ...
    log_error "Failed to start orpheus-backplane service"
    log_info "Checking service status..."
    systemctl status orpheus-backplane.service --no-pager
    exit 1
fi
...
echo "  • Service Name:    orpheus-backplane.service"
echo "  • Status:          $(systemctl is-active orpheus-backplane.service)"
...
echo "  • Check status:    systemctl status orpheus-backplane.service"
echo "  • View logs:       journalctl -u orpheus-backplane.service -f"
echo "  • Restart broker:  systemctl restart orpheus-backplane.service"
echo "  • Stop broker:     systemctl stop orpheus-backplane.service"
echo ""
echo "Next Steps:"
if [[ "$POST_INSTALL_TEST_STATUS" == "failed" || "$POST_INSTALL_TEST_STATUS" == "skipped" ]]; then
    ...
    echo "  3. Monitor logs: journalctl -u orpheus-backplane.service -f"
else
    echo "  1. Configure agents to connect to: localhost:1883"
    echo "  2. Monitor logs: journalctl -u orpheus-backplane.service -f"
fi
echo ""
